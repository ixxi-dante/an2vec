...
# Get arguments
call_impl = sys.argv[1]
assert call_impl in ['tensordot', 'whileloop']
n_xi_samples = int(sys.argv[2])
...
VAE_PARAMETERS = (
    ...
    '-dims={dims}').format(n_ξ_samples=n_ξ_samples,
                               dims=dims)
TRAINING_PARAMETERS = (
...
def p_builder(p_input):
    # CANDO: change activation
    p_layer1 = keras.layers.Dense(dim_l1, activation='relu',
                                  kernel_regularizer='l2',
                                  name='p_layer1')(p_input)
    p_adj = layers.Bilinear(0, call_impl=call_impl,
                            kernel_regularizer='l2',
