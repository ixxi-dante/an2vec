...
PATH=$PATH:~/bin OMP_NUM_THREADS=12 JULIA_NUM_THREADS=12 parallel \
        --bar --header : \
        -j6 \
        --results data/behaviour/gae-benchmarks/graphsage-git=ff73595c98-dataset=cora,citeseer-clean,pubmed-testtype=nodes,edges-testprop=0.15,0.1:0.1:0.9-diml1enc=64-dimxiadj=32-bias=false-nepochs=4-nsamples=10.csv \
